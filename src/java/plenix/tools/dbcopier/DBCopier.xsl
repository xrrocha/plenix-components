        ...
        <bean class="plenix.components.copying.jdbc.JDBCProducerFactory">
            ...
            <property name="queryExecutor">
                <bean class="plenix.components.jdbc.SQLQueryExecutorImpl">
                    ...
                    <property name="expander"><ref bean="variableExpander"/></property>
                    <xsl:if test="@fetch-size">
                        <property name="fetchSize">
                            <value><xsl:value-of select="@fetch-size"/></value>
                        </property>      
                    </xsl:if>
                </bean>
            </property>
        ...
        <bean class="plenix.components.jdbc.SQLExecutorFactoryImpl">
            ...
            <xsl:if test="@batch-size">
                <property name="batchSize">
                    <value><xsl:value-of select="@batch-size"/></value>
                </property>
            </xsl:if>
            <xsl:if test="@ignore-error">
                <property name="ignoreErrors">
                    <value><xsl:value-of select="@ignore-error"/></value>
                </property>
            </xsl:if>
        </bean>                                            
    </xsl:template>
    ...
    <xsl:template match="database">
        <bean id="{@name}DataSource" class="org.apache.commons.dbcp.BasicDataSource">
            <property name="driverClassName">
                <value><xsl:value-of select="driver/@class"/></value>
            </property>
            <property name="url">
                <value><xsl:value-of select="driver/@url"/></value>
            </property>
            <property name="username">
                <value><xsl:value-of select="user/@name"/></value>
            </property>
            <property name="password">
                <value><xsl:value-of select="user/@password"/></value>
            </property>
            <property name="defaultAutoCommit">
                <value><xsl:value-of select="@auto-commit"/></value>
            </property>
        </bean>
    </xsl:template>
